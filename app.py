...
st.set_page_config(page_title="西語全能家教 6.4", page_icon="🇪🇸", layout="wide")
...
st.sidebar.info("🚀 核心：Gemini 1.5 Flash (穩定版)")
...
if st.sidebar.button("🔔 讀取今日 A2 推播教材"):
    with st.spinner('編寫中...'):
        ...
        except: st.error("API 目前擁擠，請靜候 60 秒再點擊。")
...
if format_type == "雙人對話":
    v_a = st.sidebar.selectbox("角色 A (男)", [es_m, "es-MX-JorgeNeural"])
    v_b = st.sidebar.selectbox("角色 B (女)", [es_f, "es-MX-DaliaNeural"])
else:
    v_main = st.sidebar.selectbox("主要音色", [es_f, es_m, "es-MX-DaliaNeural"])
...
with tab1:
    st.title("🇪🇸 西語穩定版 AI 家教")
    topic = st.text_input("輸入練習主題：", placeholder="例如：仙台旅行、離岸風電挑戰...")
    ...
    if st.button("🚀 生成自訂教材"):
        if not topic: st.warning("請輸入主題")
            ...
            with st.spinner('連線中，請稍候...'):
                try:
                    p = f"老師。主題：{topic}。等級：{level}。西文長度約{word_count}字。格式：[SPANISH]原文 [CHINESE]翻譯 [VOCAB]5單字 [GRAMMAR]詳解。繁中輸出。"
...
with tab2:
    st.title("📝 實力檢測站")
    q_topic = st.text_input("測驗範圍：", key="q_topic", placeholder="輸入想考的主題")
